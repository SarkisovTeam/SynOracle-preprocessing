...
def smit_PV_extract(filename):
    Total_DATA = []
    html_doc = open(filename, encoding="UTF-8")
    ...
    bold_tag = ['1', '1′', '2', '3', '4', '5', '6', '7', '7′', '8', '(1)', '(2)', '(3)', '1a', '1b', '1c', '2a', '2b', '2c',
            ...
            while out == 0:
                ...
                if bold_index != -1 and bold_index != 0 and bold_index != len(bold_sentence):
                    ...
                    if bold_index + len(i.string) <= len(bold_sentence) and bold_sentence[
                        bold_index + len(i.string) - 1].isdigit() or bold_sentence[bold_index + len(i.string)].isalpha():
                        pass_crit += 1
                if pass_crit == 2:
    ...
    # handle table
    Pore_table = re.compile(
        '^\w?PV\s?|^V\W{0,3}T\W?|^V\W{0,3}p$|^V\W{0,3}p\W+\W+|^V\W{0,3}meso\W?|^V\W{0,3}micro\W?|^V\W{0,3}total\W?|^V\W{0,3}pore\W?|Pore\s{0,3}Volume',
        re.IGNORECASE)
    Pore = re.compile(
        'Volume\W?\(cm3\s?g\)|mesopore\W?volume|micropore\W?volume|pore\W?volume|\W+PV\s?|V\W?T|V\W?P|V\W?meso|V\W?micro|V\W?total|V\W?pore|effective\W?free\W?volume|permanent\W?porosity',
        re.IGNORECASE)
    BET_table = re.compile('SBET|BET|Brunauer\s?[−-]?\W?Emmett\s?[−-]?\W?Teller|Langmuir|Surface\s?area|m2/g|SA\W|m2 g-1',
                           re.I)
                            ...
                            if noprint == 0:
                                ...
                                if MOF_column == 1:
                                    Total_DATA.append([data[j + head_row_num][0][0], surf_column[i][1],
                                                       data[j + head_row_num][surf_column[i][0]][0], 'cm3/g'])
                                    ...
                                    Total_DATA.append(
                                        [MOF_name, surf_column[i][1], data[j + head_row_num][surf_column[i][0]][0],
                                         'cm3/g'])
                                    print(MOF_name, "|", surf_column[i][1], "|",
                                          data[j + head_row_num][surf_column[i][0]][0], "|", 'cm3/g')
                                    ...
                                    Total_DATA.append(
                                        ['*NO_MOF_data*', surf_column[i][1], data[j + head_row_num][surf_column[i][0]][0],
                                         'cm3/g'])
                                    print('*NO_MOF_data*', "|", surf_column[i][1], "|",
                                          data[j + head_row_num][surf_column[i][0]][0], "|", 'cm3/g')
                            ...
                            for j in range(len(data[i]) - 1):
                                ...
                                if noprint == 0:
                                    if MOF_column == 1:
                                        ...
                                        Total_DATA.append(
                                            [MOF_name, surf_column[i][1], data[surf_column[i][0]][j + 1][0], 'cm3/g'])
                                        print(MOF_name, "|", surf_column[i][1], "|", data[surf_column[i][0]][j + 1][0], "|",
                                              'cm3/g')
                                        ...
                                        Total_DATA.append(
                                            ['*NO_MOF_data*', surf_column[i][1], data[surf_column[i][0]][j + 1][0], 'cm3/g'])
                                        print('*NO_MOF_data*', "|", surf_column[i][1], "|", data[surf_column[i][0]][j + 1][0],
                                              "|", 'cm3/g')
                        except IndexError:
    ...
    unit = re.compile(
        'cm3\s?/\s?g|cm3\s?\W?g\s?[−-]1|/\s?g\W?cm\s?[−-]3|mL\s?/\W?g\s?|mL\s?\W?g\s?[−-]1|cm3\W?cm\s?[−-]3|cm3\s?/\W?cm3\s?')  # cm3/g criteria
    range_prob = re.compile(
        '\s?range\s?[-]?\s?of\s\d+\s?\W?\s?\d+|\s?excess\s?of\s?\d+|from\s?\d+\s?to\s?\d+\s?|from\s?\d+|over\s?\d+|\d+\s?~\s?\d+|\d+\s?–\s?\d+|as\s?high\s?as\s?\d+|∼\d+|[<>]\d+|\s?exceeding\s?\d+|\s?higher\s?than\s?\d+|\s?larger\s?than\s?\d+|theoretical\s?value\s?of\s?\d+',
        re.I)
    ...
    print(Total_DATA)
    if len(Total_DATA) > 0:
        return pd.DataFrame(Total_DATA).rename({0: 'Name',1:'Measurement', 2: 'Value', 3: 'unit'},
                                               axis=1)
    else:
...
if __name__ == "__main__":
    ...
    print(smit_PV_extract('./NIST-ZIF-8/Aic13970.html'))
