...
# quick check to see if nltk is installed
try:
    wn
except LookupError:
    import nltk
...
def smit_find_sa(filename):
    Total_DATA = []
    with open(filename, encoding="UTF-8") as html_doc:
    ...
    for i in f(["script", "style", 'ol', 'ul', 'li', 'table', 'a', 'noscript', 'option']):
        i.extract()
    for i in f(["div"]):
        # logging.debug(i.get('class'))
        if i.get('class') in ['citationInfo', 'casRecord', 'casContent', 'casTitle', 'casAuthors', 'casAbstract']:
            logging.debug(i.get_text())
            i.extract()
    ...
    front = re.compile(
        '[^A-Z]\d+.$|\s?Fig.\s?$|\s+\w{1,3}[,.]\s?$|i.e.\s?$')  # finds a numbe or "Fig" or a list of words, or "i.e."
    back = re.compile('^\s?\w?\d+|^g.\s?|^\s?\w+[,.]\s?$|^,')  # finds 'g' or other things
    ...
    for i in range(len(sentences)):
        ...
        for j in range(len(words)):
            ...
            if len(words[j]) > 2:
                if words[j][-1] == ',' or words[j][-1] == '.' or words[j][-1] == ';' or words[j][-1] == ':' or words[j][
                    -1] == "\n":
                    words[j] = words[j][:-1]
                if words[j][-1] == ')':
                    words[j] = words[j][:-1]
            ...
            if MOF_Crit > 2:
                logging.debug(f'{words[j]} = score: {MOF_Crit}')
            ...
            if words[j].find('-') != -1:
                ...
                for k in range(len(tempword)):
                    ...
                    if contin_crit == 1:
                        continue
                    # logging.debug(f'{tempword[k]}, {len(tempword[k])}')
                    if len(tempword[k]) <= 1:
                        Prop_noun = 0
                    if k != 0 and tempword[k].isdigit():
                        continue
                    # logging.debug(wn.lemmas(tempword[k]))
                    Atom_word = 0
                    if len(tempword[k]) == 2 and tempword[k].isalpha() == True and tempword[k][0].isupper() == True and \
            ...
            if MOF_Crit > 2:
                ...
                if not words[j][0].islower() or words[j][0:3] == 'sod' or words[j][0:3] == 'rho' or words[j][
                                                                                                    0:3] == 'bio' or \
                        words[
                            j].find('agglomerate') != -1:  # first letter big
    ...
    for i in f(["div"]):
        if i.get('class') in ['citationInfo', 'casRecord', 'casContent', 'casTitle', 'casAuthors', 'casAbstract']:
            #	if i.get('class') == ['citationInfo'] or i.get('class') == ['casRecord'] or i.get('class') == [
            #		'casContent'] or i.get('class') == ['casTitle'] or i.get('class') == ['casAuthors'] or i.get('class') == [
            #		'casAbstract']:
            # logging.debug(i.get_text())
            i.extract()

    # Treating Bold tag MOFs
    bold_tag = ['1', '1′', '2', '3', '4', '5', '6', '7', '7′', '8', '(1)', '(2)', '(3)', '1a', '1b', '1c', '2a', '2b',
                '2c',
                '3a', '3b', '3c', 'a', 'b', 'c', 'd', '1 a', '2 a', '2 b', 'A', 'B', 'C', 'D', '1 d', 'I', 'II']
    bold_tag_candidate = []
    bold_MOF = []
    for i in f.find_all(['strong', 'b']):
        ...
        if bold_tag_Crit == 1:
            if bold_tag_candidate != []:
                ...
                bold_tag_candidate.append(i.string)
    logging.debug(bold_tag_candidate)
    for i in f.find_all(['strong', 'b']):
            ...
            while out == 0:
                ...
                if bold_index != -1 and bold_index != 0 and bold_index != len(bold_sentence):
                    if bold_sentence[bold_index - 1].isdigit() or bold_sentence[bold_index - 1].isalpha():
                        pass_crit += 1
                    logging.debug(f'{bold_index}, {i.string}, {len(bold_sentence)}')
                    if bold_index + len(i.string) <= len(bold_sentence) and bold_sentence[
                        bold_index + len(i.string) - 1].isdigit() or bold_sentence[
                        bold_index + len(i.string)].isalpha():
                        pass_crit += 1
                if pass_crit == 2:
            ...
            for k in range(len(bold_sentence)):
                ...
                if store == 0 and bold_sentence[k].find(i.string) != -1:
                    ...
                    if k != 0 and k != len(bold_sentence) - 1:
                        ...
                        for l in range(len(MOF_in_Paper)):
                            if bold_sentence[k - 1] == 'MOF' or bold_sentence[k - 1] == '(MOF' or bold_sentence[
                                k - 1] == 'Compound' or bold_sentence[k - 1] == '(Compound':
                                if bold_sentence[k - 2].find(MOF_in_Paper[l]) != -1:
                                    store = 1
                                    MOF = bold_sentence[k - 2]
                                    break
    ...
    logging.info("MOF_Abbreviations")
    logging.info(bold_MOF)
    for i in f.find_all(['strong', 'b']):
    ...
    # handle table
    BET_table = re.compile(
        'SBET|BET|Brunauer\s?[−-]?\W?Emmett\s?[−-]?\W?Teller|Langmuir|Surface\s?area|m2/g|SA\W|m2 g-1',
        re.I)
    ...
    for table in tables:
        ...
        if data != []:
            ...
            for i in range(len(data[0])):
                ...
                if BET_table.search(surface_finder) != None:
                    if BET.search(surface_finder) != None:
                        type = 'BET'
                    elif Lang.search(surface_finder) != None:
                        type = 'Lang'
                    else:
                        type = '*IDK*'
                    head_type = 'top'
            ...
            for i in range(len(data)):
                logging.debug(i,data[i])
                if data[i] != [] and BET_table.search(data[i][0][0]) != None:
                    if BET.search(data[i][0][0]) != None:
                        type = 'BET'
                    elif Lang.search(data[i][0][0]) != None:
                        type = 'Lang'
                    else:
                        type = '*IDK*'
                    head_type = 'left'
                    surf_column.append([i, type, head_type])
                    surf_table = 1
            logging.debug(surf_column)
            if surf_table == 1 and surf_column != []:
                logging.info('<Surface area data in table>\n')
                isMOF = 0
                if head_type == 'top':
                        ...
                        for j in range(len(data) - head_row_num):
                            ...
                            if noprint == 0:
                                ...
                                if MOF_column == 1:
                                    ...
                                    logging.debug(data[j + head_row_num][0][0], "|", surf_column[i][1], "|",
                                          data[j + head_row_num][surf_column[i][0]][0], "|", 'm2/g')
                                    ...
                                    MOF_name = MOF_name + '--' + data[head_row_num - 1][0][0] + ':' + \
                                               data[head_row_num + j][0][0]
                                    ## state word need to be added
                                    Total_DATA.append(
                                        [MOF_name, surf_column[i][1], data[j + head_row_num][surf_column[i][0]][0],
                                         'm2/g'])
                                    logging.debug(MOF_name, "|", surf_column[i][1], "|",
                                          data[j + head_row_num][surf_column[i][0]][0], "|", 'm2/g')
                                    ...
                                    Total_DATA.append(
                                        ['*NO_MOF_data*', surf_column[i][1],
                                         data[j + head_row_num][surf_column[i][0]][0],
                                         'm2/g'])
                                    logging.debug('*NO_MOF_data*', "|", surf_column[i][1], "|",
                                          data[j + head_row_num][surf_column[i][0]][0], "|", 'm2/g')
                        ...
                        for j in range(len(data[i]) - 1):
                            ...
                            if noprint == 0:
                                if MOF_column == 1:
                                    ...
                                    Total_DATA.append(
                                        [MOF_name, surf_column[i][1], data[surf_column[i][0]][j + 1][0], 'm2/g'])
                                    logging.debug(MOF_name, "|", surf_column[i][1], "|", data[surf_column[i][0]][j + 1][0], "|",
                                          'm2/g')
                                    ...
                                    Total_DATA.append(
                                        ['*NO_MOF_data*', surf_column[i][1], data[surf_column[i][0]][j + 1][0], 'm2/g'])
                                    logging.debug('*NO_MOF_data*', "|", surf_column[i][1], "|",
                                          data[surf_column[i][0]][j + 1][0],
                                          "|", 'm2/g')
    ...
    if exp_crit > sim_crit:
    	logging.debug('This paper is Experiment paper\n')
    elif exp_crit == sim_crit:
    	logging.debug('Cannot recongnize Exp/Sim\n')
    else:
    	logging.debug('This paper is Simulation paper\n')

    # Surface area search
    # m2/g finding - m2/g criteria
    unit = re.compile('\s?c?m2\s?/\s?g\s?|\s?c?m2\s?\S?\s?g\s?\S?1\s?|\s+c?m2\s?g\s?')  # m2/g criteria
    range_prob = re.compile(
        '\s?range\s?[-]?\s?of\s\d+\s?\W?\s?\d+|\s?excess\s?of\s?\d+|from\s?\d+\s?to\s?\d+\s?|from\s?\d+|over\s?\d+|\d+\s?~\s?\d+|\d+\s?–\s?\d+|as\s?high\s?as\s?\d+|∼\d+|[<>]\d+|\s?exceeding\s?\d+',
        re.I)
    ...
    for i in range(len(sentences)):
        ...
        if data_sentence != []:
        	logging.debug(sentences[i]) # m2/g sentence
        	logging.debug("\n")
        ...
        for match in unit_index:
            ...
            logging.debug(match.group()) # m2/g
            logging.debug(match.span()) # m2/g index in sentence
            temptext = str(match.group())
            if temptext.find('c') != -1:
                unittext = 'cm2/g'
            else:
                unittext = 'm2/g'
            ...
            if value != None:
                                ...
                                for l in range(len(temp1)):
                                    if temp1.find(',') != -1:
                                        ...
                                        for m in range(len(temp2)):
                                            temp1 += temp2[m]
                                        isdigit = 1
                                if isdigit == 1 and float(temp1) < float(50.0):  # ignore data < 50
                                    not_store = 1
                ...
                logging.debug("No_value before_unit_m2/g")
            value_list.reverse()
                ...
                if value_list_total != [] and m2count != 1:
                    for m in range(len(value_list_total)):
                        if value_list[l] == value_list_total[m]:
                            dontadd = 1
                value_list[l] = value_list[l].replace(',', '')
                value_list[l] = value_list[l].replace(' ', '')
                if float(value_list[l]) < 100:
                    dontadd = 1
        ...
        if data_sentence != []:
            logging.debug(value_list_total)
            ...
            if BET_all == [] and Lang_all == []:
                if unit.search(sentences[i - 1]) == None:
                    BET_all = BET.findall(sentences[i - 1])
                    BET_iter = BET.finditer(sentences[i - 1])
                    Lang_all = Lang.findall(sentences[i - 1])
                    Lang_iter = Lang.finditer(sentences[i - 1])
                    if BET_all == [] and Lang_all == []:
                        Type.append("cannot_find_surface_Type")
                    elif BET_all != [] and Lang_all == []:
                        Type.append('BET')

                    elif BET_all == [] and Lang_all != []:
                        Type.append('Langmuir')
                    else:
                        BBLL_type = 1
                        sumB = 0
                        sumL = 0
                        match_numB = 0
                        match_numL = 0
                        for match in BET_iter:
                            match_numB += 1
                            sumB += (match.start() + match.end()) / 2
                        sumB = sumB / match_numB
                        for match in Lang_iter:
                            match_numL += 1
                            sumL += (match.start() + match.end()) / 2
                        sumL = sumL / match_numL
                        if abs(sumB - sumL) < 30:
                            BBLL = 'BLBL'
                        else:
                            BBLL = 'BBLL'
            ...
            if len(Type) == 0 or len(Type) == 1:
                ...
                if len(value_list_total) % 2 != 0:
                    ...
                    if len(value_list_total) == len(Type_total):
                        ...
                        logging.debug("Matching error!! even number of value needed")
                ...
                logging.debug("more than two type of surface - Check MOF Type!")
            ...
            if special_case == 0:
                    ...
                    if len(Type) == 2 and strange == 0:
                        ...
                        if BBLL_type == 0:
                            ...
                            if k < end:
                                ...
                                logging.debug(f'{MOF_list[k - end]} | {Type[1]} | {value_list_total[k]} | {unittext}')
                        elif BBLL == 'BLBL':
                            a, b = divmod(k, 2)
                            Total_DATA.append([MOF_list[a], Type[b], value_list_total[k], unittext])
                            logging.debug(f'{MOF_list[a]} | {Type[b]} | {value_list_total[k]} | {unittext}')
                ...
                if BET_count == len(MOF_list):
                    ...
                    for k in range(len(value_list_total)):
                        ...
                        logging.debug(f'{MOF_list[k]} | {Type_total[k]} | {value_list_total[k]} | {unittext}')
                else:
                    ...
                    for k in range(len(value_list_total)):
                        ...
                        logging.debug(f'{MOF_list[k]} | {Type_total[k]} | {value_list_total[k]} | {unittext}')
    if len(Total_DATA)>0:
        return pd.DataFrame(Total_DATA).rename({0: 'Name', 1:'Measurement', 2:'Value', 3:'unit'}, axis=1)
    else:
        return pd.DataFrame(columns=['Name', 'Measurement', 'Value', 'unit'])


if __name__ == "__main__":

    #filename = input("Input file name : ")
    #print(smit_find_sa('./zif_paragraphs/c0jm00500b.html')[0])
    print(smit_find_sa('./NIST-ZIF-8/Aic13970.html'))
